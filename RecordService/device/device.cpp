    ...
    Client(new DevicePrivate(this), parent)
{
    ...
    QObject::connect(d->owner, &User::userIdChanged, [this,d](const QString &userId){
        Q_UNUSED(userId);
        Q_EMIT ownerChanged(d->owner);
    });
    QObject::connect(d->owner, &User::userGroupChanged, [this,d](const QString &userGroup){
        Q_UNUSED(userGroup);
        Q_EMIT ownerChanged(d->owner);
    });
    QObject::connect(d->owner, &User::userNameChanged, [this,d](const QString &userName){
        Q_UNUSED(userName);
        Q_EMIT ownerChanged(d->owner);
    });
}
...
QString Device::vad() const
{
    return d_func()->vad;
}

int Device::percent() const
{
