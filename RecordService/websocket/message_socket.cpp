...
bool MessageSocket::active() const
{
    return d_func()->active;
}

QNetworkRequest MessageSocket::request() const
{
    return d_func()->request;
}

QString MessageSocket::error() const
{
    return d_func()->error;
}
...
void MessageSocket::setActive(const bool active)
{
    ...
    if(active != d->active){
        ...
        if(active){
            ...
            QObject::connect(d->socket, &QWebSocket::disconnected,
                             [this,d](){
                Q_EMIT disconnected();
                d->socket->deleteLater();
                d->socket = Q_NULLPTR;
            });
            QObject::connect(d->socket, static_cast<void(QWebSocket::*)(QAbstractSocket::SocketError)>(&QWebSocket::error),
            ...
            QObject::connect(d->socket, &QWebSocket::textMessageReceived,
                             [this](const QString &message){
                Q_EMIT textReceived(message);
            });
