...
void NdAccountPrivate::doRequest(NdAccountPrivate::COMMAND command)
{
    ...
    case SignOut:
        method = DELETE;
        path = QString("/v0.93/tokens/%1").arg(userMap.value(QStringLiteral("access_token")).toString());
        break;
    case QueryInfo:
        method = GET;
        path = QString("/v0.93/users/%1?realm=%2").arg(userMap.value(QStringLiteral("user_id")).toString(),
                                                               QStringLiteral("uc.sdp.nd"));

        authorization = this->encryptAuthorization(GET,
                                                   url.host(),
                                                   path,
    ...
    //the url will be encodeed when use QUrl directly, which server is not supported yet.
    QNetworkRequest request(QUrl(QStringLiteral("https://")+QStringLiteral("aqapi.101.com")+path));
...
QByteArray NdAccountPrivate::encryptAuthorization(NdAccountPrivate::METHOD method,
                                                  QString host,
                                                  QString path,
...
QString NdAccountPrivate::hmacSha256(QByteArray key, QByteArray baseString)
{
    ...
    if (key.length() > blockSize) { // if key is longer than block size (64), reduce key length with SHA256 compression
