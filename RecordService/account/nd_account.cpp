...
void NdAccount::requestFinished(QNetworkReply* reply)
{
...
QUrl NdAccount::commandToUrl(int command)
{
    QUrl uri;
    QString url;
...
void NdAccount::doRequest(int method, int command, const QByteArray &data)
{
...
void NdAccount::parseResult(bool error, int command, const QJsonDocument& jsonDocument)
{
...
void NdAccount::encryptAuthorization(int method, QString uri)
{
    QString httpMethod;
    switch(method){
    case POST:
...
QString NdAccount::encryptMD5_Salt(QString content)
{
